...
if($stmt = $conn->prepare("SELECT id, passw FROM tech_info WHERE email = ?")) {
    ...
    if($stmt->num_rows>0) {
        ...
        if($stmt = $conn->prepare("INSERT INTO tech_info (Fname, email, lab_name, tel_num, lab_code, passw) VALUES (?, ?, ?, ?, ?, ?)")) {
